...
if (defined('_Clanwars')) {
    $qry = db("SELECT s1.id,s1.datum,s1.clantag,s1.gegner,s1.url,s1.xonx,s1.liga,s1.punkte,s1.gpunkte,s1.maps,s1.serverip,
             ...
             WHERE DATE_FORMAT(FROM_UNIXTIME(s1.datum), '%d.%m.%Y') = '" . date("d.m.Y", (int)($_GET['time'])) . "'
             ORDER BY s1.datum DESC
             LIMIT " . ($page - 1) * config('m_clanwars') . "," . config('m_clanwars') . "");

    $i = $entrys - ($page - 1) * config('m_clanwars');
    $entrys = cnt($db['cw'], " WHERE DATE_FORMAT(FROM_UNIXTIME('" . $get['datum'] . "'), '%d.%m.%Y') = '" . date("d.m.Y", (int)($_GET['time'])) . "'");
    ...
    if (_rows($qry)) {
        ...
        while ($get = _fetch($qry)) {
            ...
            $gegner = show(_cw_details_gegner, array("gegner" => cut(h($get['clantag']) . " - " . h($get['gegner']), config('l_clanwars'), true, true),
