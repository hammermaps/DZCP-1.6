...
class Selection implements \Iterator, IRowContainer, \ArrayAccess, \Countable
{
	...
	protected function execute(): void
	{
		...
		if ($this->primary === null && $this->sqlBuilder->getSelect() === []) {
		...
		$this->rows = [];
		$usedPrimary = true;
		$pdoStatement = $result->getPdoStatement();
		if ($pdoStatement) {
	...
	protected function getSpecificCacheKey(): string
	{
		...
		$cols = $this->getPreviousAccessedColumns();
		return $this->specificCacheKey = $this->sqlBuilder->getSelectQueryHash(is_array($cols) ? $cols : null);
	}
	...
	{
		return $this->query($this->sqlBuilder->buildDeleteQuery())->getRowCount() ?? 0;
	}
	...
	): ?GroupedSelection
	{
		...
		assert($clone instanceof GroupedSelection);
