...
class SqlBuilder
{
	...
	{
		$query = (string) preg_replace_callback($this->getColumnChainsRegxp(), function (array $match) use (&$joins): string {
	...
	private function getConditionHash(string $condition, array $parameters): string
	{
		foreach ($parameters as $key => &$parameter) {
			if ($parameter instanceof Selection) {
				...
				$parameter = $this->getConditionHash((string) $key, (array) $parameter);
			}
		}
